...
def input_generator(x_data, y_data, batch_size, bool_data_shuffle=False):
    if bool_data_shuffle:
...
def model_train(Model, tf_model_path, tf_model_name, x_train, y_train, x_vali, y_vali, epoch_num=1000, batch_size=512,
                ...
                bool_show_proceeding_bar=True,bool_log_file=False):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch_num):
            ...
            for batch_x_train, batch_y_train in input_generator(x_train, y_train, batch_size, bool_data_shuffle=True):
                len_batch_x_train = len(batch_x_train)
                if bool_show_proceeding_bar and bool_show_epoch:
